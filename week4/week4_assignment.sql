...
-- ANSWER:
SELECT paymentDate, SUM(amount) AS `total amount`
FROM payments
...
-- ANSWER:
SELECT productCode, quantityOrdered, SUM(quantityOrdered * priceEach) AS `total price`
FROM  orderdetails
...
-- ANSWER:
SELECT checkNumber, MAX(amount) AS `highest amount`
FROM payments
